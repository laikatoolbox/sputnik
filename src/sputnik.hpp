    ...
    if (file.is_open()) {
        ...
        while (std::getline(file, line)) {
            ...
            if (sectionStart || objectStart) { // sections and objects share a lot of logic
                ...
                if (sectionStart) { // section specific logic
                    lineState = Sputnik::LineState::InSection;
                    sector->sections.insert({sectionOrObjectName, {}});
                    currentSectionOrObject = &(sector->sections[sectionOrObjectName]); 
                } else { // object specific logic
                ...
                // assign it in the current object
                currentSectionOrObject->emplace(key, value);
            }
        }
